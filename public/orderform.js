let currentSubtotal = 0, currentTotal = 0, currentFee = 0, currentRestaurant = null;
...
// Stores the current restaurant to easily retrieve data. The assumption is that this object is
// following the same format as the data included above. If you retrieve the restaurant data from
// the server and assign it to this variable, the client order form code should work automatically.
// Stored the order data. Will have a key with each item ID that is in the order, with the associated
// value being the number of that item in the order.
let order = {};
...
function selectRestaurant() {
    ...
    if (result) {
        ...
        info.innerHTML =
            currentRestaurant.name + "<br>Minimum Order: $" +
            currentRestaurant.min_order + "<br>Delivery Fee: $" +
            currentRestaurant.delivery_fee + "<br><br>";
    } else {
...
function getItemById(id) {
    ...
    for (const item of categories) {
        if (currentRestaurant.menu[item].hasOwnProperty(id)) {
            return currentRestaurant.menu[item][id];
        }
    }
    return null;
}
